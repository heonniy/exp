...
def _plot(path: Path, rows: list[dict]) -> None:
    ...
    axes[0, 1].plot(ks, [row["prefill_seconds"] for row in rows], marker="o", label="prefill")
    axes[0, 1].plot(ks, [row["decode_seconds"] for row in rows], marker="o", label="decode")
    axes[0, 1].plot(ks, [row["e2e_seconds"] for row in rows], marker="o", label="E2E")
    axes[0, 1].set(title="200-request makespan", xlabel="k", ylabel="seconds")
    axes[0, 1].legend()
    axes[1, 0].plot(ks, [row["decode_tokens_per_second"] for row in rows], marker="o", color="tab:green")
    axes[1, 0].set(title="Decode throughput", xlabel="k", ylabel="generated tokens/s")
    axes[1, 1].plot(ks, [row["prefill_h2d_fetches"] for row in rows], marker="o", label="prefill")
    axes[1, 1].plot(ks, [row["decode_h2d_fetches"] for row in rows], marker="o", label="decode")
    axes[1, 1].set(title="Expert H2D fetches", xlabel="k", ylabel="fetch count")
    axes[1, 1].legend()
